...
import com.tencent.liteav.demo.play.utils.NetWorkSpeedUtils;
...
public class SuperPlayerView extends RelativeLayout implements ITXVodPlayListener, ITXLivePlayListener {
    ...
    private int mCurrentPlayState = SuperPlayerConst.PLAYSTATE_NOTSTARTED;
...
//        popupWindow.setFocusable(true);
    ...
    public void onPlayEvent(TXVodPlayer player, int event, Bundle param) {
        //TXVodPlayer onPlayEvent event: 2007, 缓冲开始    TXVodPlayer onPlayEvent event: 2014, 缓冲结束  event：-2305, HLS解密key获取失败
        if (event == TXLiveConstants.PLAY_EVT_PLAY_LOADING) {        // 缓冲开始
            if (mPlayMode == SuperPlayerConst.PLAYMODE_WINDOW) {
                mVodControllerSmall.mLlLoading.setVisibility(VISIBLE);
            } else if (mPlayMode == SuperPlayerConst.PLAYMODE_FULLSCREEN) {
                mVodControllerLarge.mLlLoading.setVisibility(VISIBLE);
            }
        } else if (event == TXLiveConstants.PLAY_EVT_VOD_LOADING_END) {        // 缓冲结束
            if (mPlayMode == SuperPlayerConst.PLAYMODE_WINDOW) {
                mVodControllerSmall.mLlLoading.setVisibility(GONE);
            } else if (mPlayMode == SuperPlayerConst.PLAYMODE_FULLSCREEN) {
                mVodControllerLarge.mLlLoading.setVisibility(GONE);
            }
        }
        ...
        if (event < 0 && event != TXLiveConstants.PLAY_ERR_HLS_KEY
    ...
    public void onPlayEvent(int event, Bundle param) {
        ...
        } else if (event == TXLiveConstants.PLAY_EVT_PLAY_LOADING) {        // 缓冲开始
...
//        }
    ...
    public void onNetStatus(Bundle status) {
        Log.i(TAG, "播放---状态 ---" + status.toString());
...
//        void onStartFloatWindowPlay();
    ...
    public ImageView getSmallCoverPicView() {
        return mVodControllerSmall.getBackgroundView();
    }


    private ViewClickListener mListener;

    public void setViewClickListener(final ViewClickListener listener) {
        mListener = listener;
    }
    ...
    public void setDanmuData(ArrayList<String> danmuData) {
        if (mDanmuView != null) {
            mDanmuView.setDanmuData(danmuData);
        }
    }

    public void addDanmuOneData(String oneData) {
        if (mDanmuView != null) {
            mDanmuView.addDanmaku(oneData, true);
        }
    }
...
//                    Log.i(TAG, "current net speed  = " + speed);
                    mVodControllerLarge.setSpeedText(speed);
                    mVodControllerSmall.setSpeedText(speed);
                    break;
            }
