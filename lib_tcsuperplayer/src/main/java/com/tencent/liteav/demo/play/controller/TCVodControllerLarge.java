...
//    private ProgressBar mPbLiveLoading;
    ...
    public void updateVodVideoProgress() {
        float curTime = mVodController.getCurrentPlaybackTime() + 15;
        float durTime = mVodController.getDuration();

        Log.i(TAG, "速度---快进 ---" + curTime + "==" + durTime);
        if (durTime > 0 && curTime <= durTime) {
            ...
            //设置视频播放进度
//            int seekTime = (int) (percentage * durTime);
            int seekTime = (int) (progress * mVodController.getDuration());
            mVodController.seekTo((int) curTime);
            Log.i(TAG, "速度---快进 ---" + seekTime + "==" + percentage + "==" + progress + "==" + mSeekBarProgress.getMax());
            if (durTime >= 0 && curTime <= durTime) {
...
//    }
    ...
    public void onClick(View view) {
        int i = view.getId();
        if (i == R.id.iv_back || i == R.id.tv_title || i == R.id.iv_exit) {
            exitFullScreen();

        } else if (i == R.id.iv_retreat) {
    ...
    private void toggleDanmu() {
        ...
        if (mDanmukuOn) {
            ...
            mTvComment.setVisibility(VISIBLE);
        } else {
            mIvDanmuku.setImageResource(R.drawable.ic_danmuku_off);
            mTvComment.setVisibility(GONE);
        }
